...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .section { width: 100%; max-width: 1100px; margin-bottom: 8px; margin-top: 32px; }
  .section h3 { font-size: 0.75rem; font-weight: 600; letter-spacing: 0.08em;
...
<div class="grid">
  ...
  <a class="card" href="${BASE_PATH}/mcp/sse">
    <h2>MCP Server (browser)</h2>
    <p>Browser-automation MCP — Outlook, Google Messages, WhatsApp via Chrome. 12 tools over SSE.</p>
    <span class="badge">${BASE_PATH}/mcp/sse</span>
  </a>
</div>

<div class="section"><h3>Messaging (native protocol sidecars)</h3></div>
<div class="grid">
  <a class="card" href="${BASE_PATH}/whatsapp-mcp/sse">
    <h2>WhatsApp MCP</h2>
    <p>whatsmeow Go bridge — native WhatsApp protocol, sessions persist across restarts. First run: <code>docker logs -f agent-whatsapp-mcp</code> for QR.</p>
    <span class="badge">${BASE_PATH}/whatsapp-mcp/sse (SSE)</span>
  </a>
  <a class="card" href="${BASE_PATH}/gmessages/">
    <h2>Google Messages MCP</h2>
    <p>OpenMessage (libgm) — native RCS/SMS protocol. Click to open the web UI and scan the QR code with the Google Messages app on your phone.</p>
    <span class="badge">${BASE_PATH}/gmessages/mcp/sse (SSE)</span>
  </a>
</div>
</body>
</html>';
    ...
    # -------------------------------------------------------------------------
    # WhatsApp MCP sidecar (docker-compose.whatsapp-mcp.yml)
    # Uses Docker's embedded DNS resolver + set $upstream so nginx starts even
    ...
    # -------------------------------------------------------------------------
    # Google Messages MCP sidecar (docker-compose.gmessages-mcp.yml)
    # Proxies both the web UI (QR pairing) and /mcp/sse MCP endpoint.
    # -------------------------------------------------------------------------
